...
async def refresh_countries(db: Session = Depends(get_db)):
    """
    Fetch all countries and exchange rates from external APIs, then cache in database.
    ...
    - Generates a summary image with statistics
    """
    try:
    ...
    return {
        ...
        "total_countries": processed_count,
        "last_refreshed_at": refresh_timestamp.isoformat() + "Z"
    }
...
async def get_summary_image():
    """
    Serve the auto-generated summary image with country statistics.
